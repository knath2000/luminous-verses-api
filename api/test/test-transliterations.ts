...
async function testTransliterationEndpoint() {
  ...
  try {
    ...
    const singleVerse = await prisma.quranTransliteration.findFirst({
      where: {
        sura: BigInt(1),
        aya: BigInt(1)
      },
    ...
    const multipleVerses = await prisma.quranTransliteration.findMany({
      where: {
        sura: BigInt(1)
      },
      orderBy: { aya: 'asc' },
    ...
    const rangeVerses = await prisma.quranTransliteration.findMany({
      where: {
        sura: BigInt(2),
        aya: {
          gte: BigInt(1),
    ...
    const dataQualityCheck = await prisma.quranTransliteration.findMany({
      where: {
        OR: [
          { rawText: null },
          { cleanText: null },
    ...
    const performanceTest = await prisma.quranTransliteration.findMany({
      where: {
        sura: BigInt(2)
      },
      orderBy: { aya: 'asc' },
...
export { testTransliterationEndpoint };